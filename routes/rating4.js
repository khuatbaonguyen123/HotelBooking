...
router.post('/submit4', async (req, res) => {
    ...
    const dbquery = "SELECT DISTINCT booker_id FROM reservation re JOIN room_reserved rr ON rr.reservation_id = re.id JOIN room r ON r.id = rr.room_id WHERE re.booker_id = ? AND r.type_id = 4 AND re.status = 'checkout';";
