...
router.post('/submit', async (req, res) => {
    ...
    db.beginTransaction((err) => {
        ...
        db.query(`INSERT INTO rating (rating) VALUES ('${rating}')`, (err, result) => {
            if (err) {
                db.rollback(() => {
                    throw err;
                });
            ...
            db.commit((err) => {
                ...
                res.redirect('/detail1'); // Chuyển hướng sau khi lưu thành công
            });
        });
    });
});
...
router.get('/data', (req, res) => {
    const dataQuery = 'SELECT rating, COUNT(*) AS count FROM rating GROUP BY rating';
