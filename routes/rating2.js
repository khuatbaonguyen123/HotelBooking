...
router.post('/submit2', async (req, res) => {
    ...
    db.beginTransaction((err) => {
        ...
        db.query(`INSERT INTO rating2 (rating) VALUES ('${rating}')`, (err, result) => {
            if (err) {
                db.rollback(() => {
                    throw err;
                });
            ...
            db.commit((err) => {
                ...
                res.redirect('/detail2'); // Chuyển hướng sau khi lưu thành công
            });
        });
    });
});
...
router.get('/data2', (req, res) => {
    const dataQuery = 'SELECT rating, COUNT(*) AS count FROM rating2 GROUP BY rating';
