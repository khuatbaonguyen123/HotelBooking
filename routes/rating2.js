...
const Rating = require('../model_mongodb/dbmongo2.js');
...
router.post('/submit2', async (req, res) => {
    ...
    const dbquery = "SELECT DISTINCT booker_id FROM reservation re JOIN room_reserved rr ON rr.reservation_id = re.id JOIN room r ON r.id = rr.room_id WHERE re.booker_id = ? AND r.type_id = 2 AND re.status = 'checkout';";
    ...
    if (results.length === 0) {
        ...
        return res.redirect('/Assignment_r2');
    }
    ...
    try{
        for (let i = 0; i < results.length; i ++) {
            ...
            res.redirect('/Assignment_s2');
        }
    }
    catch {
        console.error('Đã đánh giá', err);
        return res.redirect('/Assignment_db2');
    }
});
