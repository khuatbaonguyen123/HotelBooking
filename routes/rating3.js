...
router.post('/submit3', async (req, res) => {
    ...
    db.beginTransaction((err) => {
        ...
        db.query(`INSERT INTO rating3 (rating) VALUES ('${rating}')`, (err, result) => {
            if (err) {
                db.rollback(() => {
                    throw err;
                });
            ...
            db.commit((err) => {
                ...
                res.redirect('/detail3'); // Chuyển hướng sau khi lưu thành công
            });
        });
    });
});
...
router.get('/data3', (req, res) => {
    const dataQuery = 'SELECT rating, COUNT(*) AS count FROM rating3 GROUP BY rating';
