...
router.post('/submit5', async (req, res) => {
    ...
    db.beginTransaction((err) => {
        ...
        db.query(`INSERT INTO rating5 (rating) VALUES ('${rating}')`, (err, result) => {
            if (err) {
                db.rollback(() => {
                    throw err;
                });
            ...
            db.commit((err) => {
                ...
                res.redirect('/detail5'); // Chuyển hướng sau khi lưu thành công
            });
        });
    });
});
...
router.get('/data5', (req, res) => {
    const dataQuery = 'SELECT rating, COUNT(*) AS count FROM rating5 GROUP BY rating';
