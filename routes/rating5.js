...
const Rating = require('../model_mongodb/dbmongo5.js');
const db = require('../database');
let cnt = 0;


router.post('/submit5', async (req, res) => {
    ...
    const dbquery = 'SELECT DISTINCT booker_id FROM reservation re JOIN room_reserved rr ON rr.reservation_id = re.id JOIN room r ON r.id = rr.room_id WHERE re.booker_id = ? AND r.type_id = 5;';
    ...
    if (results.length === 0) {
        ...
        return res.redirect('/Assignment_r5');
    }
    ...
    try{
        for (let i = 0; i < results.length; i ++) {
            ...
            res.redirect('/Assignment_s5');
        }
    }
    catch {
        console.error('Đã đánh giá', err);
        return res.redirect('/Assignment_db5');
    }
});
